window.csrfTokenCache=window.csrfTokenCache||null;let originalFetch=window.fetch||fetch;async function getCSRFToken(){if(window.csrfTokenCache)return window.csrfTokenCache;try{var e=await(await originalFetch("/app/controllers/get_csrf_token.php")).json();return window.csrfTokenCache=e.token,e.token}catch(e){return"undefined"!=typeof logger&&logger.error("Chyba získání CSRF tokenu:",e),""}}function showToast(e,t="info"){var a=document.getElementById("wgs-toast");a&&a.remove();let o=document.createElement("div");o.id="wgs-toast",o.textContent=e,o.style.cssText=`
  ...
  `,document.getElementById("wgs-toast-styles")||(a.id="wgs-toast-styles",document.head.appendChild(a)),document.body.appendChild(o),setTimeout(()=>{o.style.animation="slideOut 0.3s ease-in",setTimeout(()=>o.remove(),300)},3e3)}let WGS_DATA_CACHE=[],ACTIVE_FILTER="all",CURRENT_RECORD=null,SELECTED_DATE=null,SELECTED_TIME=null,EMAILS_S_CN=[],STAVY_NABIDEK={},CURRENT_PAGE=1,HAS_MORE_PAGES=!1,LOADING_MORE=!1,PER_PAGE=50,CAL_MONTH=(new Date).getMonth(),CAL_YEAR=(new Date).getFullYear(),SEARCH_QUERY="",WGS_ADDRESS="Dubče 364, Běchovice 190 11, Česká republika",WGS_COORDS={lat:50.0472,lng:14.5881},DISTANCE_CACHE={},Utils={getCustomerName:e=>e.jmeno||e.zakaznik||"Neznámý zákazník",getAddress:e=>e.adresa||(0<(e=[e.ulice,e.mesto,e.psc].filter(e=>e)).length?e.join(", "):"—"),getProduct:e=>e.model||e.vyrobek||"—",getOrderId:(e,t=0)=>e.cislo||e.reklamacniCislo||e.id||"WGS-"+(t+1),isCompleted:e=>"HOTOVO"===e.stav||"done"===e.stav,escapeHtml:e=>{var t=document.createElement("div");return t.textContent=e,t.innerHTML},addCountryToAddress:e=>e&&(e.toLowerCase().includes("česk")?e:e+", Česká republika"),filterByUserRole:e=>{if(!CURRENT_USER||CURRENT_USER.is_admin||"prodejce"!==CURRENT_USER.role)return e;let t=String(CURRENT_USER.id),a=(CURRENT_USER.supervised_user_ids||[]).map(String);return e.filter(e=>{e=String(e.created_by||"");return e===t||!!a.includes(e)})}},CacheManager={load:()=>{try{var e=localStorage.getItem("wgs_distance_cache");e&&(DISTANCE_CACHE=JSON.parse(e),logger.log("Načteno",Object.keys(DISTANCE_CACHE).length,"vzdáleností z cache"))}catch(e){logger.error("Chyba při načítání cache:",e)}},save:()=>{try{localStorage.setItem("wgs_distance_cache",JSON.stringify(DISTANCE_CACHE))}catch(e){logger.error("Chyba při ukládání cache:",e)}}},AUTO_REFRESH_INTERVAL=6e4,autoRefreshTimer=null,lastRefreshTime=Date.now();function startAutoRefresh(){autoRefreshTimer&&clearInterval(autoRefreshTimer),autoRefreshTimer=setInterval(async()=>{"visible"===document.visibilityState&&(logger.log("[AutoRefresh] Automaticka aktualizace dat..."),await loadAll(ACTIVE_FILTER),lastRefreshTime=Date.now())},AUTO_REFRESH_INTERVAL),logger.log(`[AutoRefresh] Spusten - interval ${AUTO_REFRESH_INTERVAL/1e3}s`)}function stopAutoRefresh(){autoRefreshTimer&&(clearInterval(autoRefreshTimer),autoRefreshTimer=null,logger.log("[AutoRefresh] Zastaven"))}function initSearch(){var e=document.getElementById("searchInput");let t=document.getElementById("searchClear");var a=new URLSearchParams(window.location.search).get("search");a&&(e.value=a,SEARCH_QUERY=a.trim().toLowerCase(),t.classList.add("visible"),window.history.replaceState({},document.title,window.location.pathname)),e.addEventListener("input",e=>{SEARCH_QUERY=e.target.value.trim().toLowerCase(),t.classList.toggle("visible",0<SEARCH_QUERY.length),renderOrders(Utils.filterByUserRole(WGS_DATA_CACHE))}),e.addEventListener("keydown",e=>{"Escape"===e.key&&clearSearch()})}function clearSearch(){var e=document.getElementById("searchInput"),t=document.getElementById("searchClear"),e=(e.value="",SEARCH_QUERY="",t.classList.remove("visible"),Utils.filterByUserRole(WGS_DATA_CACHE));renderOrders(e)}function highlightText(e,t){return Utils.highlightText?Utils.highlightText(e,t):Utils.escapeHtml(e)}function matchesSearch(e,t){return!t||[Utils.getCustomerName(e),e.telefon||"",e.email||"",Utils.getAddress(e),Utils.getProduct(e),Utils.getOrderId(e),e.popis_problemu||""].join(" ").toLowerCase().includes(t)}function initFilters(){document.querySelectorAll(".filter-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".filter-btn").forEach(e=>e.classList.remove("active")),e.classList.add("active"),ACTIVE_FILTER=e.dataset.filter,renderOrders(Utils.filterByUserRole(WGS_DATA_CACHE))})})}function updateCounts(e){var t,a,o;Array.isArray(e)&&(t=e.length,a=e.filter(e=>{var t=e.stav||"wait";return!("ČEKÁ"!==t&&"wait"!==t||(t=(e.email||"").toLowerCase().trim())&&EMAILS_S_CN.includes(t))}).length,o=e.filter(e=>{e=e.stav||"wait";return"DOMLUVENÁ"===e||"open"===e}).length,e=e.filter(e=>{e=e.stav||"wait";return"HOTOVO"===e||"done"===e}).length,document.getElementById("count-all").textContent=`(${t})`,document.getElementById("count-wait").textContent=`(${a})`,document.getElementById("count-open").textContent=`(${o})`,document.getElementById("count-done").textContent=`(${e})`)}async function loadAll(a="all",o=!1){try{var n=o?CURRENT_PAGE+1:1,i=Date.now(),r=await fetch(`app/controllers/load.php?status=${a}&page=${n}&per_page=${PER_PAGE}&_t=`+i,{cache:"no-store",headers:{"Cache-Control":"no-cache"}});if(!r.ok)throw new Error("Chyba načítání");var d=await r.json();let e=[];"success"===d.status&&Array.isArray(d.data)?e=d.data:Array.isArray(d)&&(e=d),CURRENT_PAGE=o?(WGS_DATA_CACHE=[...WGS_DATA_CACHE,...e],n):(WGS_DATA_CACHE=e,1),HAS_MORE_PAGES=e.length===PER_PAGE,LOADING_MORE=!1;var s=Utils.filterByUserRole(WGS_DATA_CACHE);updateCounts(s),renderOrders(s),updateLoadMoreButton(),lastRefreshTime=Date.now()}catch(e){logger.error("Chyba:",e),WGS_DATA_CACHE=[],document.getElementById("orderGrid").innerHTML=`
      <div class="empty-state">
        <div class="empty-state-text">${t("data_load_error")}</div>
      </div>
    `}}async function renderOrders(e=null){var a=document.getElementById("orderGrid"),o=document.getElementById("searchResultsInfo");e=e||Utils.filterByUserRole(WGS_DATA_CACHE);let n=e=Array.isArray(e)?e:[];if("all"!==ACTIVE_FILTER&&"cn"!==ACTIVE_FILTER){let a={wait:["ČEKÁ","wait"],open:["DOMLUVENÁ","open"],done:["HOTOVO","done"]};n=e.filter(e=>{var t=e.stav||"wait",t=a[ACTIVE_FILTER]?.includes(t);if("wait"===ACTIVE_FILTER&&t){e=(e.email||"").toLowerCase().trim();if(e&&EMAILS_S_CN.includes(e))return!1}return t})}var i=n.length;if(SEARCH_QUERY?(0<(n=n.filter(e=>matchesSearch(e,SEARCH_QUERY))).length?(o.className="search-results-info",o.textContent=t("search_results_found").replace("{count}",n.length).replace("{total}",i).replace("{query}",SEARCH_QUERY)):(o.className="search-results-info no-results",o.textContent=t("no_search_results").replace("{query}",SEARCH_QUERY)),o.classList.remove("hidden")):o.classList.add("hidden"),0===n.length)a.innerHTML=`
      <div class="empty-state">
        <div class="empty-state-text">${SEARCH_QUERY?t("no_results_found"):t("no_claims_to_display")}</div>
      </div>
    `;else{let b={};try{var r=await(await fetch("/api/notes_api.php?action=get_unread_counts")).json();"success"===r.status&&(b=r.unread_counts||{})}catch(e){logger.warn("Nepodařilo se načíst unread counts:",e)}try{var d=await(await fetch("/api/nabidka_api.php?action=emaily_s_nabidkou&_t="+Date.now(),{cache:"no-store",headers:{"Cache-Control":"no-cache"}})).json();"success"===d.status&&(EMAILS_S_CN=d.data?.emaily||d.emaily||[],STAVY_NABIDEK=d.data?.stavy||d.stavy||{})}catch(e){logger.warn("Nepodařilo se načíst emaily s CN:",e)}i=document.getElementById("count-cn"),r=(i&&(o=e.filter(e=>{e=(e.email||"").toLowerCase().trim();return e&&EMAILS_S_CN.includes(e)}).length,i.textContent=`(${o})`),(n="cn"===ACTIVE_FILTER?n.filter(e=>{e=(e.email||"").toLowerCase().trim();return e&&EMAILS_S_CN.includes(e)}):n).sort((e,t)=>{var a=e.stav||"wait",o=t.stav||"wait",n={"ČEKÁ":1,wait:1,"DOMLUVENÁ":2,open:2,HOTOVO:3,done:3},i=n[a]||1,n=n[o]||1;return i!==n?i-n:"ČEKÁ"===a||"wait"===a?(o=new Date(e.created_at||e.datum_reklamace||0),new Date(t.created_at||t.datum_reklamace||0)-o):"DOMLUVENÁ"===a||"open"===a?(i=e=>e.termin?new Date(e.termin+`T${e.cas_navstevy||"00:00"}:00`):new Date("9999-12-31"))(e)-i(t):(n=new Date(e.datum_dokonceni||e.completed_at||e.updated_at||e.created_at||0),new Date(t.datum_dokonceni||t.completed_at||t.updated_at||t.created_at||0)-n)}),a.innerHTML=n.map((e,t)=>{var a=Utils.getCustomerName(e),o=Utils.getProduct(e),n=formatDate(e.created_at||e.datum_reklamace),i=getStatus(e.stav),t=Utils.getOrderId(e,t);let r=Utils.getAddress(e),d=("—"!==r&&(s=r.split(",").map(e=>e.trim()),r=s.slice(0,2).join(", ")),"");"open"===i.class&&e.termin&&e.cas_navstevy&&(d=formatAppointment(e.termin,e.cas_navstevy));var s=e.id,s=b[s]||0,l=0<s,c=(e.email||"").toLowerCase().trim(),p=c&&EMAILS_S_CN.includes(c),c=STAVY_NABIDEK[c]||null,m="potvrzena"===c,c="cekame_nd"===c,a=SEARCH_QUERY?highlightText(a,SEARCH_QUERY):a,u=SEARCH_QUERY?highlightText(r,SEARCH_QUERY):r,o=SEARCH_QUERY?highlightText(o,SEARCH_QUERY):o,t=SEARCH_QUERY?highlightText(t,SEARCH_QUERY):t,g=SEARCH_QUERY&&matchesSearch(e,SEARCH_QUERY)?"search-match":"",h="status-bg-"+i.class;let v="",f=(p&&!d&&(v=c?"cn-cekame-nd":m?"cn-odsouhlasena":"ma-cenovou-nabidku"),"Poslána CN"),y="order-cn-text";return c?(f="Cekame ND",y="order-cn-text cekame-nd"):m&&(f="Odsouhlasena",y="order-cn-text odsouhlasena"),`
      <div class="order-box ${g} ${h} ${v}" data-action="showDetailById" data-id="${e.id}">
        <div class="order-header">
          <div class="order-number">${t}</div>
          <div style="display: flex; gap: 0.4rem; align-items: center;">
            <div class="order-notes-badge ${l?"has-unread pulse":""}" data-action="showNotes" data-id="${e.id}" title="${0<s?s+" nepřečtené":"Poznámky"}">
              <span class="notes-icon">✎</span>${0<s?s:""}
            </div>
    ...
    `}).join(""),Object.values(b).reduce((e,t)=>e+t,0)),d=document.getElementById("unreadNotesIndicator"),e=document.getElementById("unreadNotesCount");d&&e&&(0<r?(e.textContent=r,d.style.display="block"):d.style.display="none"),window.UNREAD_COUNTS_MAP=b}}function filterUnreadNotes(){let t=window.UNREAD_COUNTS_MAP||{};var e=WGS_DATA_CACHE.filter(e=>{e=e.id;return 0<t[e]}),a=(logger.log(`[Seznam] Filtrování nepřečtených poznámek: ${e.length} karet`),document.getElementById("orderGrid"));0===e.length?a.innerHTML=`
      <div class="empty-state">
    ...
    `};function createCustomerHeader(){var e,t,a,o,n;return CURRENT_RECORD?(e=Utils.getCustomerName(CURRENT_RECORD),t=Utils.getAddress(CURRENT_RECORD),a=CURRENT_RECORD.termin?formatDate(CURRENT_RECORD.termin):"—",o=CURRENT_RECORD.cas_navstevy||"—",n=getStatus(CURRENT_RECORD.stav),ModalManager.createHeader(e,`
    <strong>Adresa:</strong> ${t}<br>
    <strong>Termín:</strong> ${a} ${"—"!==o?"v "+o:""}<br>
    <strong>Stav:</strong> ${n.text}
  `)):""}async function showDetail(a){let e;if("string"==typeof a){if(!(e=WGS_DATA_CACHE.find(e=>e.id==a||e.reklamace_id==a)))return void wgsToast.error(t("record_not_found"))}else e=WGS_DATA_CACHE.find(e=>e.id==a.id||e.reklamace_id==a.reklamace_id)||a;autoAssignTechnician((CURRENT_RECORD=e).reklamace_id||e.cislo||e.id).catch(e=>logger.warn("Auto-assign technika se nezdařilo:",e.message));Utils.getCustomerName(e),Utils.getAddress(e),e.termin&&formatDate(e.termin),e.cas_navstevy,getStatus(e.stav);var o=Utils.isCompleted(e);let n="";n=o?`
      <div class="detail-info-box">
        <div class="detail-info-box-title">Zakázka dokončena</div>
        <div class="detail-info-box-subtitle">Vyřízeno dne ${e.updated_at?formatDate(e.updated_at):"—"} v ${(o=e.updated_at?new Date(e.updated_at):null)?o.getHours()+":"+String(o.getMinutes()).padStart(2,"0"):"—"}</div>
      </div>
      ...
      <div class="detail-buttons">
        ...
        `}
        <button class="detail-btn detail-btn-primary" data-action="showCustomerDetail" data-id="${e.id}">Detail zákazníka</button>
        ${e.original_reklamace_id?`
          <button class="detail-btn detail-btn-primary" data-action="showHistoryPDF" data-original-id="${e.original_reklamace_id}">Historie zákazníka</button>
        `:""}
        <button class="detail-btn detail-btn-primary" data-action="openKnihovnaPDF" data-id="${e.id}">KNIHOVNA PDF</button>
        <button class="detail-btn detail-btn-primary" data-action="showVideoteka" data-id="${e.id}">Videotéka</button>
      </div>
    `:`
      <div style="display: flex; flex-direction: column; gap: 0.5rem;">
        ${CURRENT_USER&&CURRENT_USER.is_admin?`
        <button class="btn" style="width: 100%; padding: 0.5rem 0.75rem; min-height: 38px; font-size: 0.85rem; background: #28a745; color: white;" data-action="vytvorCenovouNabidku" data-id="${e.id}">Vytvořit CN</button>
    `:""}
        ${CURRENT_USER&&"prodejce"===CURRENT_USER.role?"":`
        <button class="btn" style="width: 100%; padding: 0.5rem 0.75rem; min-height: 38px; font-size: 0.85rem; background: #1a1a1a; color: white;" data-action="startVisit" data-id="${e.id}">Zahájit návštěvu</button>
        <button class="btn" style="width: 100%; padding: 0.5rem 0.75rem; min-height: 38px; font-size: 0.85rem; background: #1a1a1a; color: white;" data-action="showCalendar" data-id="${e.id}">Naplánovat termín</button>
    ...
    `}
        <button class="btn" style="width: 100%; padding: 0.5rem 0.75rem; min-height: 38px; font-size: 0.85rem; background: #1a1a1a; color: white;" data-action="showCustomerDetail" data-id="${e.id}">Detail zákazníka</button>
        <button class="btn" style="width: 100%; padding: 0.5rem 0.75rem; min-height: 38px; font-size: 0.85rem; background: #1a1a1a; color: white;" data-action="showVideoteka" data-id="${e.id}">Videotéka</button>
  ...
  `;ModalManager.show(o)}function closeDetail(){ModalManager.close()}async function showHistoryPDF(e){if(e)try{logger.log("📚 Načítám historii PDF z původní zakázky: "+e);var t=await fetch("/api/get_original_documents.php?reklamace_id="+encodeURIComponent(e));if(!t.ok)throw new Error(`HTTP ${t.status}: `+t.statusText);var a,o=await t.json();if("error"===o.status){if(o.message&&o.message.includes("nebyla nalezena"))return void wgsToast.error("Původní zakázka byla smazána.\n\nHistorie PDF není k dispozici.");throw new Error(o.message||"Nepodařilo se načíst dokumenty")}o.documents&&0!==o.documents.length?(a=o.documents[0],logger.log("Otevírám PDF: "+a.file_path),zobrazPDFModal(a.file_path,e,"historie")):wgsToast.info("Historie PDF není k dispozici.\n\nPůvodní zakázka ještě nemá vytvořený PDF dokument.")}catch(e){logger.error("Chyba při načítání historie PDF:",e),wgsToast.error(`Nepodařilo se načíst historii PDF:
`+e.message)}else wgsToast.error("Chybí ID původní zakázky")}function normalizeCustomerData(e){var t,e={...e};return!e.jmeno&&e.zakaznik&&(e.jmeno=e.zakaznik),!e.zakaznik&&e.jmeno&&(e.zakaznik=e.jmeno),!e.adresa&&(e.ulice||e.mesto||e.psc)&&(t=[],e.ulice&&t.push(e.ulice),e.mesto&&t.push(e.mesto),e.psc&&t.push(e.psc),e.adresa=t.join(", ")),!e.adresa||e.ulice&&e.mesto||(t=e.adresa.split(",").map(e=>e.trim()),!e.ulice&&t[0]&&(e.ulice=t[0]),!e.mesto&&t[1]&&(e.mesto=t[1]),!e.psc&&t[2]&&(e.psc=t[2])),e}let startVisitInProgress=!1;function startVisit(a){var e,o;startVisitInProgress||(startVisitInProgress=!0,(e=WGS_DATA_CACHE.find(e=>e.id==a))?Utils.isCompleted(e)?(wgsToast.warning(t("visit_already_completed")),startVisitInProgress=!1):(e=normalizeCustomerData(e),localStorage.setItem("currentCustomer",JSON.stringify(e)),localStorage.setItem("visitStartTime",(new Date).toISOString()),o="photoSections_"+e.id,localStorage.removeItem(o),logger.log("Normalizovaná data uložena:",e),window.location.href="photocustomer.php?new=true"):(wgsToast.error(t("record_not_found")),startVisitInProgress=!1))}function vytvorCenovouNabidku(e){e?window.location.href="cenova-nabidka.php?reklamace_id="+encodeURIComponent(e):wgsToast.error("Chybí ID reklamace")}async function saveData(o,e){try{var n=await getCSRFToken();let a=new FormData;Object.keys(o).forEach(e=>{a.append(e,o[e])}),a.append("action","update"),a.append("csrf_token",n);var i=await fetch("/app/controllers/save.php",{method:"POST",body:a}),r=await i.json();if(!i.ok)throw new Error(`HTTP ${i.status}: `+(r.message||i.statusText));"success"===r.status?(Object.keys(o).forEach(e=>{var t=WGS_DATA_CACHE.find(e=>e.id==o.id);t&&"action"!==e&&(t[e]=o[e]),CURRENT_RECORD&&CURRENT_RECORD.id==o.id&&"action"!==e&&(CURRENT_RECORD[e]=o[e])}),wgsToast.success(e),await loadAll(ACTIVE_FILTER),o.id?(closeDetail(),setTimeout(()=>showDetail(o.id),100)):closeDetail()):wgsToast.error(t("error")+": "+(r.message||t("failed_to_save")))}catch(e){logger.error("Chyba při ukládání:",e),wgsToast.error(t("save_error")+": "+e.message)}}function showLoading(e=null){e=e||t("loading");var a=document.getElementById("loadingOverlay"),o=document.getElementById("loadingText");a&&o&&(o.textContent=e,a.classList.add("show"))}function hideLoading(){var e=document.getElementById("loadingOverlay");e&&e.classList.remove("show")}function showCalendar(t){var e=WGS_DATA_CACHE.find(e=>e.id==t);e&&(CURRENT_RECORD=e,SELECTED_DATE=null,SELECTED_TIME=null,e=`
    ${createCustomerHeader()}
  ...
  `,e.appendChild(a),document.createElement("div")),d=(a.className="calendar-weekdays",a.innerHTML=`<div>${t("monday")}</div><div>${t("tuesday")}</div><div>${t("wednesday")}</div><div>${t("thursday")}</div><div>${t("friday")}</div><div>${t("saturday")}</div><div>${t("sunday")}</div>`,e.appendChild(a),document.createElement("div")),a=(d.className="calendar-days",new Date(r,i,1)),o=new Date(r,i+1,0).getDate(),n=(a.getDay()+6)%7;let s=new Set;WGS_DATA_CACHE.forEach(e=>{var t,a;e.termin&&e.id!==CURRENT_RECORD?.id&&3===(e=e.termin.split(".")).length&&(t=parseInt(e[0]),a=parseInt(e[1]),e=parseInt(e[2]),a===i+1)&&e===r&&s.add(t)});for(let e=0;e<n;e++){var l=document.createElement("div");l.className="cal-day empty",d.appendChild(l)}var c=new Date;c.setHours(0,0,0,0);for(let n=1;n<=o;n++){let a=document.createElement("div");a.className="cal-day";var p=new Date(r,i,n);p.setHours(0,0,0,0);let o=p<c;o?(a.classList.add("disabled"),a.title="Nelze vybrat minulé datum",a.style.opacity="0.3",a.style.cursor="not-allowed",a.style.backgroundColor="#f0f0f0"):s.has(n)&&(a.classList.add("occupied"),a.title="Tento den má již nějaké termíny"),a.textContent=n,a.onclick=()=>{var e;o?wgsToast.warning(t("cannot_select_past_date")):(SELECTED_DATE=n+`.${i+1}.`+r,document.querySelectorAll(".cal-day").forEach(e=>e.classList.remove("selected")),a.classList.add("selected"),e="Vybraný den: "+SELECTED_DATE,document.getElementById("selectedDateDisplay").textContent=e,renderTimeGrid(),showDayBookingsWithDistances(SELECTED_DATE))},d.appendChild(a)}e.appendChild(d)}async function getDistance(t,a){var o=t+"|"+a;if(DISTANCE_CACHE[o])return DISTANCE_CACHE[o];try{let e=new AbortController;var n=setTimeout(()=>e.abort(),15e3),i=await fetchCsrfToken(),r=await fetch("/app/controllers/get_distance.php",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({origin:t,destination:a,csrf_token:i}),signal:e.signal});if(clearTimeout(n),r.ok){var d,s=await r.json();if(("success"===s.status||!0===s.success)&&s.distance)return d={km:(s.distance.value/1e3).toFixed(1),text:s.distance.text,duration:s.duration?s.duration.text:null},DISTANCE_CACHE[o]=d,CacheManager.save(),d}}catch(e){}return null}async function getDistancesBatch(e){e=e.map(e=>getDistance(e.from,e.to));return Promise.all(e)}async function showDayBookingsWithDistances(o){let n=document.getElementById("distanceInfo");var e=document.getElementById("dayBookings");if(n&&e){var i=(WGS_DATA_CACHE=Array.isArray(WGS_DATA_CACHE)?WGS_DATA_CACHE:[]).filter(e=>e.termin===o&&e.id!==CURRENT_RECORD?.id);if(i.sort((e,t)=>{e=e.cas_navstevy||"00:00",t=t.cas_navstevy||"00:00";return e.localeCompare(t)}),(r=Utils.getAddress(CURRENT_RECORD))&&"—"!==r){var r=Utils.addCountryToAddress(r),d=WGS_ADDRESS+"|"+r;if(void 0!==DISTANCE_CACHE[d]||(n.innerHTML=`<div style="text-align: center; color: var(--c-grey); font-size: 0.7rem; padding: 0.5rem;">${t("loading")}</div>`),0===i.length)getDistance(WGS_ADDRESS,r).then(e=>{n.innerHTML=e?`
          ...
          `:""}).catch(e=>{logger.error("Chyba při výpočtu vzdálenosti:",e),n.innerHTML=""}),e.innerHTML="";else{var s=[];let t=WGS_ADDRESS;for(let e=0;e<i.length;e++){var l=i[e],c=Utils.getAddress(l);c&&"—"!==c&&(c=Utils.addCountryToAddress(c),s.push({from:t,to:c,booking:l}),t=c)}s.push({from:t,to:r,booking:null,isNew:!0});var p=await getDistancesBatch(s.map(e=>({from:e.from,to:e.to})));let a=0;var m=[];for(let e=0;e<s.length;e++){var u,g,h,v=s[e],f=p[e];f&&(a+=parseFloat(f.km),u=0===e?"WGS Sídlo":Utils.getCustomerName(s[e-1].booking),g=v.isNew?Utils.getCustomerName(CURRENT_RECORD):Utils.getCustomerName(v.booking),h=v.isNew?SELECTED_TIME||"—":v.booking?.cas_navstevy||"—",m.push({from:u,to:g,time:h,km:f.km,isNew:v.isNew}))}d=m.map(e=>`
  ...
  `,0<i.length){let a='<div class="day-bookings" style="margin-top: 0.5rem;"><h4>Termíny v tento den:</h4>';i.forEach(e=>{var t=Utils.getCustomerName(e);a+=`
        <div class="booking-item" data-action="showBookingDetail" data-id="${e.id}">
          <strong>${e.cas_navstevy||"—"}</strong> — ${t}
          <span style="opacity:.7">(${Utils.getProduct(e)})</span>
        </div>`}),a+="</div>",e.innerHTML=a}else e.innerHTML=""}}else n.innerHTML="",e.innerHTML=""}}function showBookingDetail(a){let e;if("string"==typeof a||"number"==typeof a){if(!(e=WGS_DATA_CACHE.find(e=>e.id==a||e.reklamace_id==a)))return void wgsToast.error(t("record_not_found"))}else e=a;var o=Utils.getCustomerName(e),n=Utils.getAddress(e),i=Utils.getProduct(e),r=e.popis_problemu||"—",o=`
    ...
    <div class="modal-body">
      <div style="padding: 1.5rem; background: rgba(139, 0, 0, 0.05); border: 2px solid var(--c-error); margin-bottom: 1.5rem;">
        <div class="info-grid" style="font-family: 'Poppins', sans-serif;">
          ...
          <div class="info-value"><strong>${formatDate(e.termin||"")} v ${e.cas_navstevy||"—"}</strong></div>
  ...
  `;ModalManager.show(o)}function renderTimeGrid(){var a=document.getElementById("timeGrid");a.innerHTML="";let g=WGS_DATA_CACHE.filter(e=>e.termin===SELECTED_DATE&&e.cas_navstevy&&e.id!==CURRENT_RECORD?.id).sort((e,t)=>e.cas_navstevy.localeCompare(t.cas_navstevy)),h={};g.forEach(e=>{var t=Utils.getCustomerName(e);h[e.cas_navstevy]={zakaznik:t,model:Utils.getProduct(e),record:e}});for(let t=8;t<=19;t++)for(var o of[0,30]){let u=String(t).padStart(2,"0")+":"+(0===o?"00":"30"),e=document.createElement("div");e.className="time-slot",h[u]&&(e.classList.add("occupied"),e.title=h[u].zakaznik+" — "+h[u].model),e.textContent=u,e.onclick=async()=>{SELECTED_TIME=u,document.querySelectorAll(".time-slot").forEach(e=>e.classList.remove("selected")),e.classList.add("selected");var a=document.getElementById("selectedDateDisplay"),a=(a.textContent=`Vybraný termín: ${SELECTED_DATE} — `+SELECTED_TIME,a.style.display="block",document.getElementById("collisionWarning"));if(h[u]&&a){a.innerHTML=`KOLIZE: ${h[u].zakaznik} — ${h[u].model}<br><span style="font-size: 0.75rem; opacity: 0.8;">Počítám vzdálenost...</span>`,a.style.display="block";let t=h[u].record;var o=Utils.getAddress(CURRENT_RECORD),n=Utils.getAddress(t);if(o&&"—"!==o&&n&&"—"!==n)try{var i,r,d,s,l=g.findIndex(e=>e.id===t.id),c=0<l?g[l-1]:null,p=l<g.length-1?g[l+1]:null,m=await getDistance(Utils.addCountryToAddress(o),Utils.addCountryToAddress(n));let e=`KOLIZE: ${h[u].zakaznik} — `+h[u].model;m&&(e+=`<div class="collision-distance-box">Vzdálenost mezi zákazníky: <strong>${m.km} km</strong></div>`),(c||p)&&(e+='<div class="collision-context">',c&&(i=Utils.getAddress(c))&&"—"!==i&&(r=await getDistance(Utils.addCountryToAddress(i),Utils.addCountryToAddress(o)))&&(e+=`<span class="collision-route">Od ${Utils.getCustomerName(c)} (${c.cas_navstevy}): <strong>${r.km} km</strong></span>`),p&&(d=Utils.getAddress(p))&&"—"!==d&&(s=await getDistance(Utils.addCountryToAddress(o),Utils.addCountryToAddress(d)))&&(e+=`<span class="collision-route">K ${Utils.getCustomerName(p)} (${p.cas_navstevy}): <strong>${s.km} km</strong></span>`),e+="</div>"),a.innerHTML=e}catch(e){logger.error("Chyba při výpočtu vzdálenosti kolize:",e),a.innerHTML=`KOLIZE: ${h[u].zakaznik} — `+h[u].model}}else a&&(a.style.display="none")},a.appendChild(e)}}async function saveSelectedDate(){if(SELECTED_DATE&&SELECTED_TIME)if(CURRENT_RECORD){var a=(WGS_DATA_CACHE=Array.isArray(WGS_DATA_CACHE)?WGS_DATA_CACHE:[]).find(e=>e.termin===SELECTED_DATE&&e.cas_navstevy===SELECTED_TIME&&e.id!==CURRENT_RECORD.id);if(a){a=Utils.getCustomerName(a);if(!await wgsConfirm(t("confirm_appointment_collision").replace("{date}",SELECTED_DATE).replace("{time}",SELECTED_TIME).replace("{customer}",a),t("confirm_yes")||"Ano",t("confirm_no")||"Ne"))return}showLoading(t("saving_appointment"));a=performance.now();logger.log("⏱️ START ukládání termínu...");try{var e=performance.now(),o=await getCSRFToken(),n=performance.now(),i=(logger.log(`⏱️ CSRF token získán za ${(n-e).toFixed(0)}ms`),new FormData),r=(i.append("action","update"),i.append("id",CURRENT_RECORD.id),i.append("termin",SELECTED_DATE),i.append("cas_navstevy",SELECTED_TIME),i.append("stav","DOMLUVENÁ"),i.append("csrf_token",o),showLoading(t("saving_appointment_to_db")),performance.now()),d=(logger.log("⏱️ Odesílám POST request na save.php..."),await fetch("/app/controllers/save.php",{method:"POST",body:i})),s=performance.now(),l=(logger.log(`⏱️ POST request dokončen za ${(s-r).toFixed(0)}ms`),await d.json());if(!d.ok)throw new Error(`HTTP ${d.status}: `+(l.message||d.statusText));if("success"===l.status){var c=performance.now(),p=(logger.log(`⏱️ SUCCESS block started: ${(c-a).toFixed(0)}ms od začátku`),performance.now()),m=(CURRENT_RECORD.termin=SELECTED_DATE,CURRENT_RECORD.cas_navstevy=SELECTED_TIME,CURRENT_RECORD.stav="DOMLUVENÁ",performance.now()),u=(logger.log(`⏱️ CURRENT_RECORD update: ${(m-p).toFixed(0)}ms`),performance.now()),g=WGS_DATA_CACHE.find(e=>e.id===CURRENT_RECORD.id),h=(g&&(g.termin=SELECTED_DATE,g.cas_navstevy=SELECTED_TIME,g.stav="DOMLUVENÁ"),performance.now()),v=(logger.log(`⏱️ Cache update: ${(h-u).toFixed(0)}ms`),performance.now()),f=(hideLoading(),performance.now()),y=(logger.log(`⏱️ hideLoading(): ${(f-v).toFixed(0)}ms`),performance.now()),b=(logger.log(`⏱️ TĚSNĚ PŘED ALERT: ${(y-a).toFixed(0)}ms od začátku`),wgsToast.success(t("appointment_saved_success").replace("{date}",SELECTED_DATE).replace("{time}",SELECTED_TIME)),performance.now()),w=(logger.log(`⏱️ alert() dokončen: ${(b-y).toFixed(0)}ms`),performance.now()),x=(sendAppointmentConfirmation(CURRENT_RECORD,SELECTED_DATE,SELECTED_TIME).catch(e=>logger.warn("⚠ Email se nepodařilo odeslat:",e.message)),performance.now()),k=(logger.log(`⏱️ sendAppointmentConfirmation() launch: ${(x-w).toFixed(0)}ms`),performance.now());logger.log("⏱️ Aktualizuji detail...");let e=CURRENT_RECORD.id;var E=performance.now(),C=(closeDetail(),performance.now()),_=(logger.log(`⏱️ closeDetail(): ${(C-E).toFixed(0)}ms`),performance.now()),T=(setTimeout(()=>showDetail(e),100),performance.now()),R=(logger.log(`⏱️ setTimeout() scheduled: ${(T-_).toFixed(0)}ms`),performance.now()),z=(logger.log(`⏱️ Detail aktualizován za ${(R-k).toFixed(0)}ms`),performance.now());logger.log(`⏱️ CELKOVÝ ČAS: ${(z-a).toFixed(0)}ms (${((z-a)/1e3).toFixed(1)}s)`)}else hideLoading(),wgsToast.error(t("error")+": "+(l.message||t("failed_to_save")))}catch(e){hideLoading(),logger.error("Chyba při ukládání:",e),wgsToast.error(t("save_error")+": "+e.message);n=performance.now();logger.log(`⏱️ Čas do chyby: ${(n-a).toFixed(0)}ms`)}}else wgsToast.warning(t("no_record_to_save"));else wgsToast.warning(t("select_date_and_time"))}function showContactMenu(e){var t=CURRENT_RECORD.telefon||"",a=Utils.getAddress(CURRENT_RECORD),e=`
    ${createCustomerHeader()}
    ...
    <div class="modal-body">
      <div class="detail-buttons">
        ...
        ${a&&"—"!==a?`<a href="https://waze.com/ul?q=${encodeURIComponent(a)}&navigate=yes" class="detail-btn detail-btn-primary" style="text-decoration: none;" target="_blank">Navigovat (Waze)</a>`:""}
        ${a&&"—"!==a?`<a href="https://www.google.com/maps?q=${encodeURIComponent(a)}&layer=c" class="detail-btn detail-btn-primary" style="text-decoration: none;" target="_blank">Google Street View</a>`:""}
        <button class="detail-btn detail-btn-primary" data-action="showDetail">Zpět</button>
      </div>
    </div>
  `;ModalManager.show(e)}function toggleMap(){var e=document.getElementById("mapContent"),t=document.getElementById("mapToggleIcon");e.classList.contains("active")?(e.classList.remove("active"),t.classList.remove("active")):(e.classList.add("active"),t.classList.add("active"),e.dataset.loaded||(e.dataset.loaded="true",loadMapAndRoute()))}async function loadMapAndRoute(){var a=document.getElementById("mapContainer");if(CURRENT_RECORD){var e=Utils.getAddress(CURRENT_RECORD);if(e&&"—"!==e){e=Utils.addCountryToAddress(e);try{var o=encodeURIComponent(WGS_ADDRESS),n=encodeURIComponent(e);a.innerHTML=`
  ...
  `,document.createElement("button"));d.textContent="Stáhnout",d.style.cssText="padding: 12px 24px; font-size: 0.9rem; font-weight: 600; background: #444; color: white; border: none; border-radius: 6px; cursor: pointer;",d.onclick=()=>{var e=document.createElement("a");e.href=a,e.download=`WGS_PDF_${o||"dokument"}.pdf`,e.click()};let s=document.createElement("button");s.textContent="Sdílet",s.style.cssText="padding: 12px 24px; font-size: 0.9rem; font-weight: 600; background: #333; color: white; border: none; border-radius: 6px; cursor: pointer;",s.onclick=async()=>{if(navigator.share)try{s.textContent="Načítám...";var e=await(await fetch(a)).blob(),t=new File([e],`WGS_PDF_${o||"dokument"}.pdf`,{type:"application/pdf"});await navigator.share({files:[t],title:n})}catch(e){"AbortError"!==e.name&&wgsToast.error("Chyba: "+e.message)}finally{s.textContent="Sdílet"}else wgsToast.info("Sdílení není podporováno. Použijte tlačítko Stáhnout.")};var l=document.createElement("button");l.textContent="Zavřít",l.style.cssText="padding: 12px 24px; font-size: 0.9rem; font-weight: 600; background: #666; color: white; border: none; border-radius: 6px; cursor: pointer;",l.onclick=()=>t.remove(),r.appendChild(d),r.appendChild(s),r.appendChild(l),t.appendChild(e),t.appendChild(i),t.appendChild(r),e.querySelector("#pdfCloseBtn").onclick=()=>t.remove();let c=e=>{"Escape"===e.key&&(t.remove(),document.removeEventListener("keydown",c))};document.addEventListener("keydown",c),document.body.appendChild(t)}async function zobrazKnihovnuPDF(u){if(u){let t=document.createElement("div");t.id="knihovnaPdfOverlay",t.style.cssText=`
    position: fixed; top: 0; left: 0; right: 0; bottom: 0;
  ...
  `,e.appendChild(i),e.appendChild(a),e.appendChild(r),t.appendChild(e);let o=()=>t.remove(),n=(i.querySelector("#knihovnaCloseBtn").onclick=o,t.onclick=e=>{e.target===t&&o()},r.querySelector("#btnZavritKnihovnu").onclick=o,e=>{"Escape"===e.key&&(o(),document.removeEventListener("keydown",n))});async function g(){try{var e=await(await fetch(`/api/documents_api.php?action=seznam&reklamace_id=${encodeURIComponent(u)}&_t=`+Date.now(),{cache:"no-store"})).json();if("success"!==e.status)throw new Error(e.message||"Chyba při načítání");var t=e.dokumenty||e.data?.dokumenty||[];0===t.length?a.innerHTML=`
      ...
      `,document.body.appendChild(r),document.getElementById("wgsPromptInput"));i=r.querySelector(".wgs-prompt-btn-potvrdit"),e=r.querySelector(".wgs-prompt-btn-zrusit");setTimeout(()=>{d.focus(),d.select()},50);let s=e=>{r.remove(),t(e)};i.onclick=()=>s(d.value),e.onclick=()=>s(null),d.addEventListener("keydown",e=>{"Enter"===e.key?(e.preventDefault(),s(d.value)):"Escape"===e.key&&(e.preventDefault(),s(null))}),r.addEventListener("click",e=>{e.target===r&&s(null)})}))("Zadejte nový název dokumentu:",o,{titulek:"Prejmenovat dokument",btnPotvrdit:"Uložit",btnZrusit:"Zrušit"});if(null!==i)if(""===i.trim())wgsToast.error("Název nemůže být prázdný");else if(i.trim()!==o)try{var r=await getCSRFToken(),d=new FormData;d.append("action","prejmenovat"),d.append("dokument_id",a),d.append("novy_nazev",i.trim()),d.append("csrf_token",r);var s=await(await fetch("/api/documents_api.php",{method:"POST",body:d})).json();if("success"!==s.status)throw new Error(s.message||"Chyba při přejmenování");wgsToast.success("Dokument prejmenovan"),n&&n()}catch(e){logger.error("Chyba při přejmenování dokumentu:",e),wgsToast.error("Chyba: "+e.message)}}else if("smazatPdf"===t){o=m.getAttribute("data-id"),a=m.getAttribute("data-nazev");if(await wgsConfirm(`Opravdu chcete smazat dokument "${a}"?`,{titulek:"Smazat dokument",btnPotvrdit:"Smazat",nebezpecne:!0})){i=o;try{var l=await getCSRFToken(),c=new FormData;c.append("action","smazat"),c.append("dokument_id",i),c.append("csrf_token",l);var p=await(await fetch("/api/documents_api.php",{method:"POST",body:c})).json();if("success"!==p.status)throw new Error(p.message||"Chyba při mazání");wgsToast.success("Dokument smazan")}catch(e){logger.error("Chyba při mazání dokumentu:",e),wgsToast.error("Chyba: "+e.message)}await 0,g()}}}}))}catch(e){logger.error("Chyba při načítání dokumentů:",e),a.innerHTML=`
        <div style="text-align: center; padding: 40px; color: #dc3545;">
          Chyba: ${escapeHtml(e.message)}
        </div>
      `}}document.addEventListener("keydown",n),r.querySelector("#btnNahratPdf").onclick=()=>{zobrazFormularNahraniPdf(u,()=>g())},document.body.appendChild(t),g()}else wgsToast.error("Chybí ID zakázky")}function zobrazFormularNahraniPdf(r,d){let t=document.createElement("div"),s=(t.style.cssText=`
    position: fixed; top: 0; left: 0; right: 0; bottom: 0;
  ...
  `,e=(ModalManager.show(e),await getNotes(n.id)),e=(e.sort((e,t)=>new Date(t.timestamp)-new Date(e.timestamp)),`
    ...
    <div class="modal-body">
      <div class="notes-container">
        ${0<e.length?e.map(e=>{var t=CURRENT_USER&&(CURRENT_USER.is_admin||e.author===CURRENT_USER.email),a=e.has_audio&&e.audio_url,o="[Hlasová poznámka]"===e.text||"[Hlasova poznamka]"===e.text;return`
  ...
  `);ModalManager.show(e),setTimeout(()=>{document.querySelectorAll(".note-audio-player").forEach(t=>{t.onerror=function(){logger.log("[Audio] Chyba pri nacitani ulozene nahravky");var e=t.closest(".note-audio");e&&(e.innerHTML='<span style="color: var(--c-grey); font-size: 0.75rem;">Audio nelze nacist</span>')}})},100),setTimeout(async()=>{await markNotesAsRead(n.id),await loadAll(ACTIVE_FILTER),window.WGSNotifikace&&window.WGSNotifikace.aktualizovat()},1e3)}async function saveNewNote(e){var a=document.getElementById("newNoteText").value.trim(),o=window.wgsAudioRecorder?window.wgsAudioRecorder.audioBlob:null;if(a||o)try{await addNote(e,a,o),window.wgsAudioRecorder&&(window.wgsAudioRecorder.audioBlob=null,window.wgsAudioRecorder.audioChunks=[]),closeNotesModal(),await loadAll(ACTIVE_FILTER),window.WGSNotifikace&&window.WGSNotifikace.aktualizovat()}catch(e){wgsToast.error(t("note_save_error")+": "+e.message)}else wgsToast.warning(t("write_note_text"))}function closeNotesModal(){window.wgsAudioRecorder&&window.wgsAudioRecorder.isRecording&&stopRecording(),"function"==typeof releaseMicrophone&&releaseMicrophone(),closeDetail(),renderOrders()}async function checkMicrophonePermission(){try{var e;if(navigator.permissions&&navigator.permissions.query)return e=await navigator.permissions.query({name:"microphone"}),logger.log("[Audio] Stav opravneni mikrofonu:",e.state),"granted"===e.state?(window.wgsAudioRecorder.permissionGranted=!0,"granted"):"denied"===e.state?"denied":"prompt"}catch(e){logger.log("[Audio] Permissions API neni podporovano, zkusim primo")}return"unknown"}async function startRecording(e){logger.log("[Audio] Spoustim nahravani...");try{if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia)throw new Error("Vas prohlizec nepodporuje nahravani zvuku");var t=await checkMicrophonePermission();if("denied"===t)throw new Error("Pristup k mikrofonu byl trvale odepren. Povolte ho v nastaveni prohlizece.");window.wgsAudioRecorder.permissionGranted||"granted"===t||localStorage.getItem("wgs_mic_explained")||(wgsToast.info("Pro nahravani hlasovych poznamek potrebujeme pristup k mikrofonu. Po kliknuti na OK vas prohlizec pozada o povoleni."),localStorage.setItem("wgs_mic_explained","1"));var o=await navigator.mediaDevices.getUserMedia({audio:!0}),n=(window.wgsAudioRecorder.stream=o,window.wgsAudioRecorder.permissionGranted=!0,/^((?!chrome|android).)*safari/i.test(navigator.userAgent)||/iPad|iPhone|iPod/.test(navigator.userAgent));let e="audio/webm",a=(n?(MediaRecorder.isTypeSupported("audio/mp4")?e="audio/mp4":MediaRecorder.isTypeSupported("audio/aac")&&(e="audio/aac"),logger.log("[Audio] Safari detekovan, preferuji MP4")):MediaRecorder.isTypeSupported("audio/webm;codecs=opus")?e="audio/webm;codecs=opus":MediaRecorder.isTypeSupported("audio/webm")?e="audio/webm":MediaRecorder.isTypeSupported("audio/mp4")?e="audio/mp4":MediaRecorder.isTypeSupported("audio/ogg")&&(e="audio/ogg"),logger.log("[Audio] Pouzivam format:",e),window.wgsAudioRecorder);a.mimeType=e,a.mediaRecorder=new MediaRecorder(o,{mimeType:e}),a.audioChunks=[],a.isRecording=!0,a.recordingStartTime=Date.now(),a.mediaRecorder.ondataavailable=e=>{0<e.data.size&&(a.audioChunks.push(e.data),logger.log("[Audio] Data chunk:",e.data.size,"bytes"))},a.mediaRecorder.onstop=()=>{var e;logger.log("[Audio] Nahravani ukonceno, chunks:",a.audioChunks.length),0===a.audioChunks.length?(logger.error("[Audio] Zadna data nebyla nahrana"),wgsToast.warning("Nahravka je prazdna. Zkuste to prosim znovu."),document.getElementById("btnStartRecord").classList.remove("hidden"),document.getElementById("recordingIndicator").classList.remove("active")):(e=a.mimeType||"audio/webm",a.audioBlob=new Blob(a.audioChunks,{type:e}),a.isRecording=!1,logger.log("[Audio] Blob vytvoren:",a.audioBlob.size,"bytes, type:",e),releaseMicrophone(),showAudioPreview(a.audioBlob))},a.mediaRecorder.start(1e3),document.getElementById("btnStartRecord").classList.add("hidden"),document.getElementById("recordingIndicator").classList.add("active"),a.recordingTimer=setInterval(()=>{var e=Math.floor((Date.now()-a.recordingStartTime)/1e3),t=Math.floor(e/60),e=e%60;document.getElementById("recordingTime").textContent=t+":"+e.toString().padStart(2,"0")},1e3),logger.log("[Audio] Nahravani spusteno")}catch(e){logger.error("[Audio] Chyba pri nahravani:",e),releaseMicrophone(),"NotAllowedError"===e.name||"PermissionDeniedError"===e.name?wgsToast.error("Pristup k mikrofonu byl odepren. Povolte pristup v nastaveni prohlizece."):wgsToast.error("Chyba pri nahravani: "+e.message)}}function stopRecording(){logger.log("[Audio] Zastavuji nahravani...");let t=window.wgsAudioRecorder;if(t.recordingTimer&&(clearInterval(t.recordingTimer),t.recordingTimer=null),t.mediaRecorder&&t.isRecording)if("recording"===t.mediaRecorder.state)try{t.mediaRecorder.requestData(),setTimeout(()=>{t.mediaRecorder&&"recording"===t.mediaRecorder.state&&(t.mediaRecorder.stop(),logger.log("[Audio] MediaRecorder zastaven po requestData zpozdeni"))},150)}catch(e){logger.log("[Audio] requestData neni podporovano:",e.message),t.mediaRecorder.stop()}else"inactive"!==t.mediaRecorder.state&&t.mediaRecorder.stop();var e=document.getElementById("recordingIndicator");e&&e.classList.remove("active")}function releaseMicrophone(){var e=window.wgsAudioRecorder;e.stream&&(e.stream.getTracks().forEach(e=>{e.stop(),logger.log("[Audio] Track zastaven:",e.kind)}),e.stream=null),e.mediaRecorder&&(e.mediaRecorder=null),e.isRecording=!1,e.audioChunks=[],logger.log("[Audio] Mikrofon a MediaRecorder uvolneny")}function showAudioPreview(e){var t=URL.createObjectURL(e);let a=document.getElementById("audioPreviewPlayer"),o=document.getElementById("audioPreview"),n=(a.onerror=null,a.oncanplay=null,!1);a.onerror=function(e){n||(n=!0,logger.log("[Audio] Chyba pri nacitani nahravky:",e),o.classList.add("hidden"),document.getElementById("btnStartRecord").classList.remove("hidden"),a.src&&(URL.revokeObjectURL(a.src),a.src=""),logger.error("[Audio] Nahravka se nepodarila nacist"))},a.src=t,o.classList.remove("hidden"),logger.log("[Audio] Nahled zobrazen, velikost:",Math.round(e.size/1024),"KB")}function deleteAudioPreview(){var e=window.wgsAudioRecorder,e=(e.audioBlob=null,e.audioChunks=[],document.getElementById("audioPreviewPlayer")),t=document.getElementById("audioPreview");e.src&&(URL.revokeObjectURL(e.src),e.src=""),t.classList.add("hidden"),document.getElementById("btnStartRecord").classList.remove("hidden"),logger.log("[Audio] Nahled smazan")}function formatDateTime(e){var e=new Date(e),t=new Date-e;return t<6e4?"Právě teď":t<36e5?`Před ${Math.floor(t/6e4)} min`:t<864e5?`Před ${Math.floor(t/36e5)} h`:["ne","po","ut","st","ct","pa","so"][e.getDay()]+` ${e.getDate()}.${e.getMonth()+1}.${e.getFullYear()} ${e.getHours().toString().padStart(2,"0")}:`+e.getMinutes().toString().padStart(2,"0")}function getStatus(e){return{"ČEKÁ":{class:"wait",text:"NOVÁ"},wait:{class:"wait",text:"NOVÁ"},"DOMLUVENÁ":{class:"open",text:"DOMLUVENÁ"},open:{class:"open",text:"DOMLUVENÁ"},HOTOVO:{class:"done",text:"HOTOVO"},done:{class:"done",text:"HOTOVO"}}[e]||{class:"wait",text:"NOVÁ"}}function formatDate(e){var t;return e?(t=new Date(e),isNaN(t)?e:["ne","po","ut","st","ct","pa","so"][t.getDay()]+` ${t.getDate()}.${t.getMonth()+1}.`+t.getFullYear()):"—"}function formatAppointment(e,t){var a,o;return e&&t?3!==(o=e.split(".")).length?e+" "+t:(e=parseInt(o[0]),a=parseInt(o[1]),o=parseInt(o[2]),["ne","po","út","st","čt","pá","so"][new Date(o,a-1,e).getDay()]+` ${e}.${a}.-`+t):""}function showDeleteConfirmModal(o){return new Promise(e=>{let t=document.createElement("div");t.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.85);z-index:10003;display:flex;align-items:center;justify-content:center;";var a=document.createElement("div");a.style.cssText="background:#1a1a1a;padding:25px;border-radius:12px;max-width:400px;width:90%;text-align:center;box-shadow:0 10px 40px rgba(0,0,0,0.5);border:1px solid #333;",a.innerHTML=`
    ...
    `,t.appendChild(a),document.body.appendChild(t),document.getElementById("deleteFotoNo").onclick=()=>{logger.log("[smazatFotku] Uživatel zrušil"),document.body.removeChild(t),e(!1)},document.getElementById("deleteFotoYes").onclick=async()=>{logger.log("[smazatFotku] Uživatel potvrdil, mazám..."),document.body.removeChild(t),await pokracovatSmazaniFotky(o,n),e(!0)}})}async function pokracovatSmazaniFotky(e,a){logger.log("🗑️ Mazání fotky ID:",e);try{var o,n=await getCSRFToken(),i=new FormData,r=(i.append("photo_id",e),i.append("csrf_token",n),await fetch("api/delete_photo.php",{method:"POST",body:i})),d=await r.json();if(!r.ok)throw new Error(`HTTP ${r.status}: `+(d.message||d.error||r.statusText));if("success"===d.status){logger.log("Fotka smazána!");for(o of document.querySelectorAll(".foto-wrapper img"))if(o.src.includes(a.replace(/\\/g,""))){o.closest(".foto-wrapper").remove();break}var s,l,c=document.querySelector('[style*="Fotografie"]');c&&(s=document.querySelectorAll(".foto-wrapper").length,c.textContent=`Fotografie (${s})`,0===s)&&(l=c.closest("div").querySelector('[style*="grid"]'))&&(l.innerHTML=`<p style="color: var(--c-grey); text-align: center; padding: 1rem; font-size: 0.9rem;">${t("no_photos")}</p>`),wgsToast.success(t("photo_deleted_successfully"))}else{var p=d.message||d.error||t("delete_failed");logger.error("Chyba:",p),wgsToast.error(t("error")+": "+p)}}catch(e){logger.error("Chyba při mazání fotky:",e),wgsToast.error(t("photo_delete_error")+": "+e.message)}}async function loadPhotosFromDB(e){try{var t,a=await fetch("api/get_photos_api.php?reklamace_id="+e);return a.ok?(t=await a.json()).success&&t.photos?t.photos.map(e=>({id:e.id,photo_path:e.photo_path,section_name:e.section_name})):[]:[]}catch(e){return logger.error("Chyba načítání fotek:",e),[]}}function otevritVyberFotek(e){logger.log("[Fototeka] Otviram vyber fotek pro reklamaci:",e);e=document.getElementById("fototeka-input-"+e);e?e.click():logger.error("[Fototeka] Input element nenalezen")}async function zpracujVybraneFotky(a){var a=a.target,o=a.getAttribute("data-reklamace-id"),n=a.files;if(n&&0!==n.length){logger.log("[Fototeka] Vybrano souboru:",n.length,"pro reklamaci:",o);let e=document.getElementById("fototeka-nahravani"),t=document.getElementById("fototeka-progress");e&&(e.style.display="block"),t&&(t.style.width="0%");try{var i=[];for(let e=0;e<n.length;e++){var r,d,s,l=n[e];l.type.startsWith("image/")?15728640<l.size?wgsToast.error("Soubor "+l.name+" je prilis velky (max 15MB)"):(d=await souborNaBase64(r=await komprimujObrazek(l,1200,.3)),i.push({type:"image",data:d,size:r.size}),t&&(s=Math.round((e+1)/n.length*50),t.style.width=s+"%")):logger.warn("[Fototeka] Preskakuji neobrazovy soubor:",l.name)}if(0===i.length)throw new Error("Zadne platne fotky k nahrani");var c=await getCSRFToken(),p=await fetch("/app/save_photos.php",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reklamace_id:o,sections:{customer_detail:i},csrf_token:c})}),m=(t&&(t.style.width="80%"),await p.json());if(!p.ok||!m.success)throw new Error(m.error||"Chyba pri nahravani");t&&(t.style.width="100%"),logger.log("[Fototeka] Nahrano fotek:",m.count),wgsToast.success("Nahrano "+m.count+" fotek"),await aktualizujFototekaGrid(o),a.value=""}catch(e){logger.error("[Fototeka] Chyba pri nahravani:",e),wgsToast.error("Chyba: "+e.message)}finally{setTimeout(()=>{e&&(e.style.display="none"),t&&(t.style.width="0%")},1e3)}}else logger.log("[Fototeka] Zadne soubory vybrane")}function souborNaBase64(o){return new Promise((e,t)=>{let a=new FileReader;a.onload=()=>e(a.result),a.onerror=e=>t(e),a.readAsDataURL(o)})}function komprimujObrazek(d,s=1200,l=.3){return new Promise(r=>{var e=new FileReader;e.onload=e=>{let i=new Image;i.onload=async()=>{let t=document.createElement("canvas");var e=t.getContext("2d"),a=Math.min(1,s/Math.max(i.width,i.height));t.width=i.width*a,t.height=i.height*a,e.drawImage(i,0,0,t.width,t.height);let o=.7,n=await new Promise(e=>t.toBlob(e,"image/jpeg",o));for(;n.size>1024*l*1024&&.3<o;)o-=.05,n=await new Promise(e=>t.toBlob(e,"image/jpeg",o));a=Math.round(d.size/1024),e=Math.round(n.size/1024);logger.log(`[Fototeka] Komprese: ${a} KB -> ${e} KB (kvalita ${Math.round(100*o)}%)`),r(n)},i.onerror=()=>{logger.warn("[Fototeka] Nelze nacist obrazek pro kompresi, vracim original"),r(d)},i.src=e.target.result},e.onerror=()=>r(d),e.readAsDataURL(d)})}async function aktualizujFototekaGrid(e){try{var t=await loadPhotosFromDB(e),a=document.getElementById("fototeka-grid"),o=document.getElementById("fototeka-nadpis");a&&(o&&(o.textContent="Fototeka ("+t.length+")"),0<t.length?a.innerHTML=t.map((e,t)=>{var a="object"==typeof e?e.photo_path:e,e="object"==typeof e?e.id:null,o=a.replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/'/g,"\\'");return`
        ...
        `}).join(""):a.innerHTML='<p style="color: #666; font-size: 0.85rem; margin: 0; padding: 0.5rem 0;">Zadne fotografie</p>')}catch(e){logger.error("[Fototeka] Chyba pri aktualizaci gridu:",e)}}async function loadMoreOrders(){var e;!LOADING_MORE&&HAS_MORE_PAGES&&(LOADING_MORE=!0,(e=document.getElementById("loadMoreBtn"))&&(e.disabled=!0,e.textContent=t("loading")),await loadAll(ACTIVE_FILTER,!0))}function updateLoadMoreButton(){let e=document.getElementById("loadMoreBtn");var a;e||(a=document.getElementById("orderGrid"))&&a.parentElement&&((e=document.createElement("button")).id="loadMoreBtn",e.className="load-more-btn",e.textContent=t("load_more_orders"),e.onclick=loadMoreOrders,a.parentElement.appendChild(e)),e&&(e.classList.toggle("hidden",!HAS_MORE_PAGES),e.disabled=LOADING_MORE,e.textContent=LOADING_MORE?t("loading"):t("load_more_page").replace("{page}",CURRENT_PAGE+1))}async function sendContactAttemptEmail(t,a){try{if(WGS_DATA_CACHE.find(e=>e.id==t||e.reklamace_id==t)){showLoading("Odesílám email zákazníkovi... Prosím čekejte");var e=await getCSRFToken(),o=await(await fetch("/api/send_contact_attempt_email.php",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reklamace_id:t,csrf_token:e})})).json();if(hideLoading(),o.success){logger.log("Email o pokusu o kontakt odeslán zákazníkovi"),closeDetail(),"undefined"!=typeof WGSToast?WGSToast.zobrazit("Email odeslán zákazníkovi",{titulek:"WGS"}):showToast("Email odeslán zákazníkovi","success");let t=o.sms_text||"Dobrý den, pokusili jsme se Vás kontaktovat. Zavolejte prosím zpět na +420 725 965 826. Děkujeme, WGS Service";setTimeout(()=>{var e=encodeURIComponent(t);window.location.href=`sms:${a}?body=`+e},2e3)}else logger.error("⚠ Chyba při odesílání emailu:",o.error||o.message),showToast(o.error||"Chyba při odesílání emailu","error")}else showToast("Záznam nenalezen","error")}catch(e){logger.error("Chyba při odesílání kontaktního emailu:",e),showToast("Nepodařilo se odeslat email","error"),hideLoading()}}async function zobrazVideotekaArchiv(r){if(logger.log("[Videotéka] Otevírám archiv pro zakázku ID: "+r),document.getElementById("videotekaOverlay"))logger.log("[Videotéka] Overlay už existuje, ignoruji");else{let t=document.createElement("div");t.id="videotekaOverlay",t.style.cssText="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 10004; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 1rem;";var e=document.createElement("div"),d=(e.style.cssText="width: 95%; max-width: 900px; height: 90%; background: #222; border-radius: 8px; overflow: hidden; display: flex; flex-direction: column;",window.innerWidth<600),s=document.createElement("div");s.style.cssText=d?"padding: 12px 16px; background: #333; color: white; font-weight: 600; display: flex; flex-direction: column; align-items: center; text-align: center; gap: 2px; border-bottom: 2px solid #444;":"padding: 16px 20px; background: #333; color: white; font-weight: 600; font-size: 1rem; display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #444;",s.innerHTML="<span>Načítání...</span>";let a=document.createElement("div"),o=(a.id="videotekaContent",a.style.cssText="flex: 1; overflow-y: auto; padding: 16px; background: #1a1a1a; display: flex; flex-direction: column; gap: 12px; align-content: start; position: relative; transition: background 0.2s ease;",document.createElement("div")),n=(o.id="videotekaDropOverlay",o.style.cssText="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: rgba(45, 80, 22, 0.3); border: 3px dashed #2D5016; display: none; align-items: center; justify-content: center; z-index: 10; pointer-events: none;",o.innerHTML='<div style="color: white; font-size: 1.2rem; font-weight: 600; text-align: center; padding: 2rem;">Pusťte video pro nahrání</div>',0);a.addEventListener("dragenter",e=>{e.preventDefault(),e.stopPropagation(),n++,o.classList.remove("hidden"),a.style.background="#252525"}),a.addEventListener("dragleave",e=>{e.preventDefault(),e.stopPropagation(),0===--n&&(o.classList.add("hidden"),a.style.background="#1a1a1a")}),a.addEventListener("dragover",e=>{e.preventDefault(),e.stopPropagation()}),a.addEventListener("drop",async e=>{e.preventDefault(),e.stopPropagation(),n=0,o.classList.add("hidden"),a.style.background="#1a1a1a";var e=e.dataTransfer.files;0<e.length&&((e=e[0]).type.startsWith("video/")?(logger.log(`[Videotéka] Drag & drop: ${e.name} (${(e.size/1024/1024).toFixed(2)} MB)`),await nahratVideoDragDrop(e,r,t)):showToast("Lze nahrát pouze video soubory","error"))});try{var l,c=await(await fetch("/api/video_api.php?action=list_videos&claim_id="+r)).json();"success"===c.status&&(p=c.customer_name||"Neznámý zákazník",m=c.reklamace_cislo||r,s.innerHTML=d?`
        ...
        `,a.appendChild(l)))}catch(e){logger.error("[Videotéka] Chyba při načítání videí:",e),s.innerHTML="<span>Chyba</span>",a.innerHTML=`
  ...
  `,document.createElement("div"));var a=n?100:120,r=n?60:68,a=(t.style.cssText=`
    ...
    overflow: hidden;
    position: relative;
  `,t.innerHTML=`<span style="font-size: ${n?"1.5rem":"2rem"}; opacity: 0.5; color: #fff;">▶</span>`,t.onclick=()=>prehratVideo(o.video_path,o.video_name),generujNahledVidea(o.video_path,a,r).then(e=>{e&&(t.innerHTML=`
  ...
  `;let c=document.createElement("div");c.style.cssText="display: none; margin-bottom: 16px;";var o=document.createElement("div");o.style.cssText="width: 100%; height: 24px; background: #1a1a1a; border-radius: 4px; overflow: hidden; border: 1px solid #555;";let p=document.createElement("div"),m=(p.style.cssText="height: 100%; background: #2D5016; width: 0%; transition: width 0.3s ease; display: flex; align-items: center; justify-content: center; color: white; font-size: 0.8rem; font-weight: 600;",o.appendChild(p),c.appendChild(o),document.createElement("div"));m.style.cssText="text-align: center; color: #999; font-size: 0.85rem; margin-top: 8px; display: none;",c.appendChild(m);o=document.createElement("div");o.style.cssText="display: flex; gap: 12px; justify-content: flex-end;";let u=document.createElement("button"),g=(u.textContent="Zrušit",u.style.cssText="padding: 10px 20px; background: #666; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.9rem;",u.onclick=()=>s.remove(),document.createElement("button")),n=(g.textContent="Nahrát",g.style.cssText="padding: 10px 20px; background: #2D5016; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.9rem; font-weight: 600;",g.onclick=async()=>{var a=l.files[0];if(a){var e=524288e3<a.size;g.disabled=!0,u.disabled=!0,c.classList.remove("hidden"),m.classList.remove("hidden");try{let t=a;if(e){m.textContent="Komprimuji video...",p.style.width="10%",p.textContent="10%";try{if("undefined"==typeof MediaRecorder)throw new Error("MediaRecorder není podporován");t=await komprimovatVideo(a,e=>{e=Math.round(10+40*e);p.style.width=e+"%",p.textContent=e+"%"}),logger.log(`[Videotéka] Video komprimováno: ${a.size} → ${t.size} bytů`)}catch(e){if(logger.warn("[Videotéka] Komprese selhala, používám originál: "+e.message),t=a,m.textContent="Komprese nedostupná, nahrávám originál...",524288e3<a.size)throw showToast("Video je příliš velké (max 500 MB). Komprese selhala.","error"),new Error("Video příliš velké a komprese není dostupná")}}m.textContent="Nahrávám video...",p.style.width="50%",p.textContent="50%";var o=new FormData,n=(o.append("action","upload_video"),o.append("claim_id",r),o.append("video",t,t.name||a.name),o.append("csrf_token",document.querySelector('input[name="csrf_token"]')?.value||""),await fetch("/api/video_api.php",{method:"POST",body:o})),i=(p.style.width="90%",p.textContent="90%",await n.json());if("success"!==i.status)throw new Error(i.message||"Chyba při nahrávání");p.style.width="100%",p.textContent="100%",m.textContent="Hotovo!",p.style.background="#333","undefined"!=typeof WGSToast?WGSToast.zobrazit("Video bylo úspěšně nahráno",{titulek:"WGS"}):showToast("Video bylo úspěšně nahráno","success"),setTimeout(()=>{s.remove(),d.remove(),zobrazVideotekaArchiv(r)},1e3)}catch(e){logger.error("[Videotéka] Chyba při uploadu:",e),p.style.background="#c33",m.textContent="Chyba: "+e.message,g.disabled=!1,u.disabled=!1,showToast("Chyba při nahrávání videa: "+e.message,"error")}}else wgsToast.warning("Vyberte video soubor")},o.appendChild(u),o.appendChild(g),e.appendChild(t),e.appendChild(a),e.appendChild(l),e.appendChild(c),e.appendChild(o),s.appendChild(e),e=>{"Escape"!==e.key||g.disabled||(s.remove(),document.removeEventListener("keydown",n))});document.addEventListener("keydown",n),document.body.appendChild(s)}async function nahratVideoDragDrop(t,a,o){logger.log("[Videotéka] Zahajuji drag & drop upload: "+t.name);let n=document.createElement("div");n.style.cssText="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.95); z-index: 10007; display: flex; align-items: center; justify-content: center; padding: 1rem;";var e=document.createElement("div"),i=(e.style.cssText="background: #2a2a2a; border-radius: 8px; padding: 24px; max-width: 400px; width: 100%; border: 2px solid #444; text-align: center;",document.createElement("div")),r=(i.style.cssText="color: white; font-size: 1rem; font-weight: 600; margin-bottom: 16px;",i.textContent="Nahrávání videa...",document.createElement("div"));r.style.cssText="width: 100%; height: 24px; background: #1a1a1a; border-radius: 4px; overflow: hidden; border: 1px solid #555;";let d=document.createElement("div");d.style.cssText="height: 100%; background: #2D5016; width: 0%; transition: width 0.3s ease; display: flex; align-items: center; justify-content: center; color: white; font-size: 0.8rem; font-weight: 600;",d.textContent="0%";var s=document.createElement("div");s.style.cssText="color: #999; font-size: 0.85rem; margin-top: 12px;",s.textContent=t.name,r.appendChild(d),e.appendChild(i),e.appendChild(r),e.appendChild(s),n.appendChild(e),document.body.appendChild(n);try{let e=t;524288e3<t.size&&(s.textContent="Komprimuji video...",d.style.width="10%",d.textContent="10%",e=await komprimovatVideo(t,e=>{e=Math.round(10+40*e);d.style.width=e+"%",d.textContent=e+"%"}),logger.log(`[Videotéka] Video komprimováno: ${t.size} → ${e.size} bytů`)),s.textContent="Odesílám na server...",d.style.width="50%",d.textContent="50%";var l=new FormData,c=(l.append("action","upload_video"),l.append("claim_id",a),l.append("video",e,e.name||t.name),l.append("csrf_token",document.querySelector('input[name="csrf_token"]')?.value||""),await fetch("/api/video_api.php",{method:"POST",body:l})),p=(d.style.width="90%",d.textContent="90%",await c.json());if("success"!==p.status)throw new Error(p.message||"Chyba při nahrávání");d.style.width="100%",d.textContent="100%",s.textContent="Hotovo!",d.style.background="#333","undefined"!=typeof WGSToast?WGSToast.zobrazit("Video bylo úspěšně nahráno",{titulek:"WGS"}):showToast("Video bylo úspěšně nahráno","success"),setTimeout(()=>{n.remove(),o.remove(),zobrazVideotekaArchiv(a)},1e3)}catch(e){logger.error("[Videotéka] Chyba při drag & drop uploadu:",e),d.style.background="#c33",d.style.width="100%",s.textContent="Chyba: "+e.message,showToast("Chyba při nahrávání videa: "+e.message,"error"),setTimeout(()=>{n.remove()},3e3)}}async function komprimovatVideo(u,g){return new Promise((p,m)=>{try{let c=document.createElement("video");c.preload="metadata",c.muted=!0,c.playsInline=!0,c.onloadedmetadata=()=>{var e=c.videoWidth,n=c.videoHeight;let i=e,r=n;(1920<e||1080<n)&&(l=Math.min(1920/e,1080/n),i=Math.round(e*l),r=Math.round(n*l));e=document.createElement("canvas");e.width=i,e.height=r;let d=e.getContext("2d");var t,n=e.captureStream(30);let s="";for(t of["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4"])if(MediaRecorder.isTypeSupported(t)){s=t;break}if(s){logger.log("[Videotéka] Používám kodek: "+s);var l={mimeType:s,videoBitsPerSecond:25e5};let t;try{t=new MediaRecorder(n,l)}catch(e){t=new MediaRecorder(n,{mimeType:s})}let a=[],o=(t.ondataavailable=e=>{0<e.data.size&&a.push(e.data)},t.onstop=()=>{var e=new Blob(a,{type:s}),t=s.includes("mp4")?"mp4":"webm";e.name=u.name.replace(/\.[^.]+$/,"")+"_compressed."+t,p(e)},t.onerror=e=>{m(new Error("Chyba při kompresi videa: "+(e.error?.message||"neznámá")))},t.start(1e3),c.play().catch(e=>{m(new Error("Nelze přehrát video pro kompresi: "+e.message))}),()=>{var e;c.paused||c.ended?setTimeout(()=>t.stop(),100):(d.drawImage(c,0,0,i,r),g&&(e=c.currentTime/c.duration,g(e)),requestAnimationFrame(o))});c.onplay=()=>{o()},c.onerror=()=>{m(new Error("Chyba při načítání videa pro kompresi"))}}else m(new Error("Žádný video kodek není podporován"))},c.onerror=()=>{m(new Error("Video soubor nelze načíst"))},c.src=URL.createObjectURL(u)}catch(e){m(e)}})}window.wgsAudioRecorder={mediaRecorder:null,audioChunks:[],audioBlob:null,isRecording:!1,recordingStartTime:null,recordingTimer:null,permissionGranted:!1,stream:null},document.addEventListener("DOMContentLoaded",()=>{document.addEventListener("click",e=>{var e=e.target.closest("[data-action]");e&&(e=e.getAttribute("data-action"),["openPDF","openKnihovnaPDF","startVisit","showCalendar","vytvorCenovouNabidku","showContactMenu","showCustomerDetail","closeDetail","deleteReklamace","showDetailById","showDetail","showNotes","closeNotesModal","deleteNote","saveNewNote","showHistoryPDF","showVideoteka","saveSelectedDate","previousMonth","nextMonth","showBookingDetail","showCalendarBack","openCalendarFromDetail","sendContactAttemptEmail","showPhotoFullscreen","smazatFotku","saveAllCustomerData","startRecording","stopRecording","deleteAudioPreview","closeErrorModal","filterUnreadNotes","otevritVyberFotek"].includes(e)||("reload"===e?location.reload():"function"==typeof window[e]&&window[e]()))}),document.addEventListener("click",e=>{e=e.target.closest("[data-navigate]")?.getAttribute("data-navigate");e&&("function"==typeof navigateTo?navigateTo(e):location.href=e)}),document.addEventListener("change",e=>{var t,e=e.target.closest("[data-onchange]");e&&(t=e.getAttribute("data-onchange"),e=e.getAttribute("data-onchange-value")||e.value,"function"==typeof window[t])&&window[t](e)})}),window.otevritVyberFotek=otevritVyberFotek,window.zpracujVybraneFotky=zpracujVybraneFotky;