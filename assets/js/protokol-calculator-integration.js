...
// ========================================
// DEBUG: Detekce dvojitého volání nextStep
// ========================================
(function(){
    const orig = window.nextStep;
    if (typeof orig !== "function") {
        console.warn('[DEBUG] window.nextStep není definován při načtení protokol-calculator-integration.js');
        return;
    }
    ...
    window.nextStep = function(...args){
        ...
        console.log("[DEBUG] nextStep volání #" + callCount + ", krok=" + (window.stav?.krok || '?') + ", Δms=" + delta.toFixed(1));
        console.trace("[DEBUG] stack");

        // Pojistka: Zamknout dvojité volání v rámci jednoho ticku
        if (locked) {
            console.warn("[DEBUG] ZABLOKOVÁNO dvojité volání nextStep!");
            return;
        }
        ...
        try {
            ...
            // Odemknout v dalším ticku
            setTimeout(() => { locked = false; }, 0);
        }
    };

    console.log('[DEBUG] nextStep wrapper nainstalován');
})();
