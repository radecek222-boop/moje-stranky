...
const Utils = {
  ...
  // Přidání Česká republika k adrese
  ...
  filterByUserRole: (items) => {
    ...
    return items.filter(x => x.zpracoval_id === CURRENT_USER.id);
  }
};
...
function renderOrders(items = null) {
  ...
  grid.innerHTML = filtered.map((rec, index) => {
    ...
    const notes = getNotes(rec.id);
    const unreadCount = notes.filter(n => !n.read && n.authorId !== CURRENT_USER.id).length;
    const hasUnread = unreadCount > 0;
    
    // Zvýraznění vyhledávaných výrazů
...
function showCustomerDetail(id) {
  ...
  // Nová pole podle novareklamace.html
...
async function saveAllCustomerData(id) {
  // Standardizované názvy polí podle novareklamace.html
  const data = {
    action: 'update',
    id: id,
    // Základní údaje
...
// Označit poznámky jako přečtené
...
// Zobrazit poznámky (async version)
...
// Uložit novou poznámku (async version)
